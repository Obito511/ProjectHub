...
export default function MemberProjects() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:9090/api/projects/member/my-projects`,
        {
      ...
      const formattedProjects = response.data.map(project => ({
        ...
        status: "Active", // Add status if your API doesn't provide it
        issues: project.members?.length || 0, // Example value
        owner: project.owner,
  ...
  return (
    ...
    {project.members?.slice(0, 3).map((member, index) => (
      <img 
        key={index}
        src={member.profilePicture || `https://i.pravatar.cc/32?img=${index}`}
        alt={`avatar-${index}`}
        className="avatar"
